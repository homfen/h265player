...
export default class CommonProcessor extends BaseClass {
  ...
  provide(size) {
    ...
    while (this.source.length > 0) {
      ...
      audioTime =
        tmpBuffer.startTime +
        (tmpBuffer.duration * loadedPosition) / tmpBuffer.length;
      let copyLength = Math.min(
        tmpBuffer.length - loadedPosition,
        sourceSize - sourcePosition
      );
      let leftBuffer = tmpBuffer.getChannelData(0);
      let rightBuffer =
        tmpBuffer.numberOfChannels > 1
          ? tmpBuffer.getChannelData(1)
          : tmpBuffer.getChannelData(0);
      //fill channel data with buffer
      leftSource.set(
        leftBuffer.slice(loadedPosition, loadedPosition + copyLength),
        sourcePosition
      );
      rightSource.set(
        rightBuffer.slice(loadedPosition, loadedPosition + copyLength),
        sourcePosition
      );
      copySize += copyLength;
