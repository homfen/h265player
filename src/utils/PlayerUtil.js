...
export default class PlayerUtil {
  static createContainer(player) {
    ...
    $ele.style.width =
      (player.options.width ? player.options.width : player.el.offsetWidth) +
      'px';
    $ele.style.height =
      (player.options.height ? player.options.height : player.el.offsetHeight) +
      'px';
    return $ele;
  }
  ...
  static createScreenContainer(player) {
    ...
    let screenHeight;
    if (player.controlBarAutoHide || player.controlBar === false) {
